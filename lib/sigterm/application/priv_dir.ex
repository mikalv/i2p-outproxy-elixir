defmodule Sigterm.Application.PrivDir do
  alias Sigterm.Utils
  alias __MODULE__
  ...
  @type t :: %__MODULE__{
    ...
    hash: String.t,
    path: String.t,
    files: Sigterm.Application.PrivDir.File.t
  }
  ...
  def path(app) do
    path =
      Sigterm.Application.lib_dir(app)
      |> Path.join("priv")
    if File.dir?(path) do
      path
    else
  ...
  def compare(%__MODULE__{files: sources} = s, %__MODULE__{files: targets}) do
    files =
      Sigterm.Application.PrivDir.File.compare(sources, targets)
      |> Enum.map(fn
