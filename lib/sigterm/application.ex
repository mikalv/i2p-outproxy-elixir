defmodule Sigterm.Application do
  alias Sigterm.Utils
  ...
  def applications(app) do
    ...
    #|> Enum.map(&Sigterm.Application.load/1)
  end

  def expand_applications([], l), do: List.flatten(l)
  def expand_applications(list, loaded) do
    list =
      Enum.map(list, &Sigterm.Application.applications/1)
      |> List.flatten
  ...
  def modules(app) do
    spec(app)[:modules]
    |> Enum.map(&Sigterm.Application.Module.load(app, &1))
  end

  def priv_dir(app) do
    Sigterm.Application.PrivDir.load(app)
  end
    ...
    modified =
      Enum.reduce(sources, [], fn(s, acc) ->
        ...
        case compare(s, t) do
          ...
          {mod, s} ->
            modules =
              Sigterm.Application.Module.compare(s.modules, t.modules)
              |> Enum.map(fn
                {action, mod} when action in [:modified, :inserted] ->
                  {action, %{mod | binary: Sigterm.Application.Module.bin(s.name, mod)}}
                mod -> mod
              end)
            priv_dir = Sigterm.Application.PrivDir.compare(s.priv_dir, t.priv_dir)
  ...
  def apply({:modified, app}, overlay_path) do
    ...
    Sigterm.Application.PrivDir.apply(app.priv_dir, overlay_path)
    Enum.each(app.modules, &Sigterm.Application.Module.apply(&1, overlay_path))
