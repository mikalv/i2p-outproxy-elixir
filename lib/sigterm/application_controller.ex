defmodule Sigterm.ApplicationController do
  use GenServer
  @timeout 30_000
  alias Sigterm.Utils
  ...
  def init(opts) do
    ...
    handler = opts[:handler] || Sigterm.Handler
  ...
  def handle_info(:init, s) do
    for app <- s.init do
      case app do
        ...
        init_call ->
          IO.puts """
          Sigterm encountered an error while trying to call #{inspect init_call}
  ...
  # Sigterm Application Start Phase
  ...
  defp build_hash(application_list) do
    application_list
    #|> Sigterm.Application.expand_applications(application_list)
  ...
  def app(nil) do
    IO.puts "[Sigterm] app undefined. Finished booting"
    :proxy
  end
  def app(app) do
    if Sigterm.Application.exists?(app) do
      app
    else
      IO.puts "[Sigterm] app undefined. Finished booting"
      :proxy
    ...
    Enum.filter(apps, fn
      app when is_atom(app) ->
        if Sigterm.Application.exists?(app) do
          ...
          IO.puts "[Sigterm] Init app #{inspect app} undefined. Skipping"
          false
        end
      _ -> true
    end)
